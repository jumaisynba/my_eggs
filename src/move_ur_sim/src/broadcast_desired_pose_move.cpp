...
//            ROS_INFO("%lf",X_BEGING);
//           ROS_INFO("in transient 3%lf,%lf,%lf",Z_INIT,X_INIT,Y_INIT);

    	  ROS_INFO("compare z  now > init %lf,%lf",Z_NOW, Z_INIT);
      PoseShow_test.pose_now();
    ros::spinOnce();

   while( (ros::ok())&&(X_NOW>X_2) ){ 
           PoseShow_test.pose_now();
...
//        ROS_INFO("%lf,%lf,%lf",X_INIT,X_2,X_NOW);
     ...
     loop_rate.sleep();
             ...
             ROS_INFO("in after transient 3%lf,%lf,%lf",Z_INIT,X_INIT,Y_INIT);
	...
	{
	  	  ROS_INFO(" now > DOWn false %lf,%lf",Z_NOW, Z_DOWN);
...
//        ROS_INFO("%lf,%lf,%lf",X_INIT,X_2,X_NOW);
	...
	{	       
	  ROS_INFO("in Z_NOW > Z_DOWN false %lf,%lf",Z_NOW,Z_DOWN);
    ...
    ros::spinOnce();
     loop_rate.sleep();
           ...
           ROS_INFO("init%lf,down%lf,now%lf",Z_INIT,Z_DOWN,Z_NOW);
...
//        ROS_INFO("%lf,%lf,%lf",X_INIT,X_2,X_NOW);
	...
	{	ROS_INFO("compare z  now > down %lf,%lf",Z_NOW, Z_DOWN);
	  ...
	  { 
	    ...
	    Experiment.send_state(STATE_NUMBER);
	    Experiment.send_velocity(in_velocitydown_slow);
	    if ((Experiment.real_total_force_ur_ >= 500)||
	      (Experiment.highest_force_cell_ur_ >= 100))
	    {
	      STATE_NUMBER = 2;
	      Experiment.send_state(STATE_NUMBER);
	      Experiment.send_velocity(in_velocityup);
