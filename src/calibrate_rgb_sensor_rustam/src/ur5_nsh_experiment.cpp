...

{
	...
	while( (ros::ok()))
	{
		...
		if (!up_button_pressed && down_button_pressed && Z_NOW>0.1 && !goUp){
			
			Experiment.send_velocity(in_velocitydown_slow);
			ROS_INFO("Going DOWN");
		}else if(!up_button_pressed && down_button_pressed && goUp){
			goUp = false;
			Experiment.send_velocity(in_velocityzero);
			ROS_INFO("Stopped, can go down now!");
		}
		else if (up_button_pressed && !down_button_pressed && !goUp){
			goUp = up_button_pressed;
			
		}else if (goUp && Z_NOW<0.2){
		...
		ROS_INFO("Z_NOW before experiment: %f", Z_NOW); 
		ros::spinOnce();
		loop_rate.sleep();
	}
	Experiment.send_velocity(in_velocityzero);
